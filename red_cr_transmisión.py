...
def Z_base(bus):
    ...
    return Z_base
def correccion_pu(pu_nuevo, s_nominal):
    pu_viejo = (pu_nuevo*s_nominal)/100 #100 mva base
    return pu_viejo
...
for i in range(1, len(base_datos['Bus Data'])):
    ...
    buses[bus_index] = pp.create_bus(Red_CR1,
                                     ...
                                     name = datos_elem('Bus Data', i, 1).strip(),
...
for i in range(1, len(base_datos['Trafos'])):
    bus_1 = int(buses[str(datos_elem('Trafos', i, 0)).strip()])
    bus_2 =int(buses[str(datos_elem('Trafos', i, 1)).strip()])
    
    pp.create_switch(Red_CR1, bus=bus_1, element=bus_2, et='b',closed=True,z_ohm=0.1)
...
for i in range(1, len(base_datos['Trafos 3W'])):
    ...
    if str(datos_elem('Trafos 3W', i, 10)) not in nombres_autotrafo:
        bus_1 = int(buses[str(datos_elem('Trafos 3W', i, 0)).strip()])
        bus_2 = int(buses[str(datos_elem('Trafos 3W', i, 1)).strip()])
        bus_3 = int(buses[str(datos_elem('Trafos 3W', i, 2)).strip()])
        pp.create_switch(Red_CR1, bus=bus_1, element=bus_2, et='b',closed=True,z_ohm=0.1)
        pp.create_switch(Red_CR1, bus=bus_1, element=bus_3, et='b',closed=True,z_ohm=0.1)
        continue
...
with pd.ExcelWriter('Resultados_transmisión.xlsx', engine='xlsxwriter') as writer:
