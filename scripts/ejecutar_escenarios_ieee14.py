...
Ejecuta los 3 escenarios IEEE 14 definidos en escenarios_ieee14.py,
...
escenarios_ieee14.py en la misma carpeta.
"""
import json
import pandas as pd
...
from escenarios_ieee14 import (
    construir_escenarios,
    detectar_violaciones,
    V_MIN_OK, V_MAX_OK, CARGA_MAX_OK,
)
...
def color_bus(v_pu):
    ...
    if v_pu < V_MIN_OK or v_pu > V_MAX_OK:
...
# ==============================================================
def generar_tabla_resumen(escenarios):
    """
    Construye un DataFrame con el resumen de los 3 escenarios.
    ...
    for nombre, net in escenarios.items():
        ...
        sub = v['barras_subtension']
        sobretension = v['barras_sobretension']
        sobrec = v['lineas_sobrecarga']
 
        # Listar buses/líneas en violación
        buses_sub = ", ".join([f"B{i+1}" for i in sub.index]) if len(sub) else "—"
        buses_sobretension = ", ".join([f"B{i+1}" for i in sobretension.index]) if len(sobretension) else "—"
        lineas_sob = ", ".join([
            f"L{net.line.loc[i,'from_bus']+1}-{net.line.loc[i,'to_bus']+1}"
            for i in sobrec.index
        ]) if len(sobrec) else "—"
 
        filas.append({
            'Escenario': nombre.upper(),
            'Vmin [pu]': round(v['vmin'], 4),
            'Bus Vmin': f"B{v['bus_vmin']}",
            'Vmax [pu]': round(v['vmax'], 4),
            'Bus Vmax': f"B{v['bus_vmax']}",
            'Cargabilidad máx [%]': round(v['loading_max'], 2),
            'N° barras subtensión': len(sub),
            'Buses en subtensión': buses_sub,
            'N° barras sobretensión': len(sobretension),
            'Buses en sobretensión': buses_sobretension,
            'N° líneas sobrecarga': len(sobrec),
...
def main():
    ...
    # Paso 1: construir los 3 escenarios
    print("\n[1/3] Construyendo escenarios...")
    escenarios = construir_escenarios()
    for nombre in escenarios:
        print(f"  ✓ Escenario '{nombre}' construido")
 
    # Paso 2: imprimir resumen por escenario
    print("\n[2/3] Detectando violaciones operativas...")
    for nombre, net in escenarios.items():
        v = detectar_violaciones(net)
        print(f"\n  [{nombre.upper()}]")
        print(f"    Vmin = {v['vmin']:.4f} pu (bus {v['bus_vmin']})")
        print(f"    Vmax = {v['vmax']:.4f} pu (bus {v['bus_vmax']})")
        print(f"    Cargabilidad máx = {v['loading_max']:.2f}%")
